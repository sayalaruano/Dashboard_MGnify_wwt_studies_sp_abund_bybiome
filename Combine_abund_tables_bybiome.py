...
biome_count = studies_data["biome"].value_counts()

# Rename the entries in the biomes column
studies_data['biome'] = studies_data['biome'].replace({
...
# Count the number of studies per biome
biome_count_new = studies_data["biome"].value_counts()

# Get the lists of study ids for each biome
biome_list = studies_data["biome"].unique()
...
for biome in biome_list:
    biome_dict[biome] = studies_data[studies_data["biome"] == biome]["study_id"].tolist()
...
global_abund_dfs_species = []
global_taxa_dfs_phylum = []
global_taxa_dfs_genus = []
global_taxa_dfs_species = []
global_metadata_dfs = []
...
for biome, study_ids in biome_dict.items():
    ...
    if abund_dfs_species:
        ...
        # Save the DataFrame to a csv file
        merged_abund_df_species.to_csv(f'Abundance_tables/Merged_tables/{biome}/{biome}_merged_abund_tables_species.csv')
        ...
        # Save the DataFrame to a csv file
        studies_per_sample_species_df.to_csv(f'Abundance_tables/Merged_tables/{biome}/{biome}_studies_per_sample.csv', index=False)
    ...
    if taxa_dfs_species:
        ...
        # Save the DataFrame to a csv file
        merged_taxa_df_species.to_csv(f'Abundance_tables/Merged_tables/{biome}/{biome}_merged_taxa_tables_species.csv')
    ...
    global_abund_dfs_species.extend(abund_dfs_species)
    global_taxa_dfs_phylum.extend(taxa_dfs_phylum)
    global_taxa_dfs_genus.extend(taxa_dfs_genus)
    global_taxa_dfs_species.extend(taxa_dfs_species)
...
# Concatenate and aggregate species-level abundance data for all biomes
if global_abund_dfs_species:
    merged_global_abund_df_species = pd.concat(global_abund_dfs_species, axis=1)
    merged_global_abund_df_species = merged_global_abund_df_species.fillna(0).astype(int)
    otu_index = ['OTU' + str(i+1) for i in range(len(merged_global_abund_df_species))]
    merged_global_abund_df_species.index = otu_index
    merged_global_abund_df_species.index.name = 'OTU'
    merged_global_abund_df_species.reset_index(inplace=True)
    merged_global_abund_df_species.to_csv('Abundance_tables/Merged_tables/All_biomes/all_biomes_merged_abund_tables_species.csv', index=False)
...
# Concatenate and deduplicate species-level taxonomic data for all biomes
if global_taxa_dfs_species:
    merged_global_taxa_df_species = pd.concat(global_taxa_dfs_species).drop_duplicates()
    merged_global_taxa_df_species.reset_index(inplace=True, drop=True)
    merged_global_taxa_df_species.to_csv('Abundance_tables/Merged_tables/All_biomes/all_biomes_merged_taxa_tables_species.csv', index=False)
